...
void ReceiveThread(SOCKET clientSocket) {
    ...
    while (true) {
        ...
        if (bytesReceived > 0) {
            ...
            response1.data[0] = buffer[0+2];
            response1.data[1] = buffer[1 + 2];
            response1.crc.data_t[0] = buffer[4];
            response1.crc.data_t[1] = buffer[5];
